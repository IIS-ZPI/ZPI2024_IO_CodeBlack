...
class TestCurrencyManager(unittest.TestCase):
    ...
    @patch("builtins.print")
    @patch("builtins.input", side_effect=["fetch-data", "USD", "2024-01-02", "2024-01-04", "exit"])
    ...
    def test_change_histogram_invalid_pair(self, mock_print, mock_input, mock_show):
        from main import main
        main()
        self.assertIn("Invalid format. Use exactly one '/' like USD/EUR.", [call.args[0] for call in mock_print.call_args_list])
    ...
    def test_change_histogram_invalid_period(self, mock_print, mock_input, mock_show):
        from main import main
        main()
        self.assertIn("Invalid period. Use '1m' for one month or '1q' for one quarter.", [call.args[0] for call in mock_print.call_args_list])
